...
interface JQuery {
  ...
  //tagsManager(options?: any): any;
}

interface JQuery {
  typeahead(options: any, dataset: any[]): JQuery;
  typeahead(val: any, dataset: any): JQuery;
  typeahead(options: any): JQuery;
  // jQuery.fn.typeahead.noConflict()
}

interface JQuery {
  //source: (query: string, cb: (result: any) => void) => void | Bloodhound<any>;
}
